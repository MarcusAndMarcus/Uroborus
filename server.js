...
const CFG = {
  ...
  geminiModel: (process.env.GEMINI_MODEL || 'gemini-3-flash').trim(),
  geminiThinking: (process.env.GEMINI_THINKING || 'minimal').trim(),
  arbiterModel: (process.env.ARBITER_MODEL || '').trim(),      // vazio = mesmo do CLAUDE_MODEL
  ...
  accessToken: (process.env.ACCESS_TOKEN || '').trim(),         // vazio = aberto (só faça isso em rede local)
...
function priceOf(m) {
  const exato = Object.keys(PRICES).find((k) => m.startsWith(k));
  if (exato) return PRICES[exato];
  /* modelo novo, fora da tabela: estima pelo mais caro da família. Subestimar o custo
     furaria o teto de gasto silenciosamente; superestimar apenas para mais cedo. */
  if (/^gemini/i.test(m)) return { in: 1.25, out: 10.00, estimado: true };
  if (/^claude/i.test(m)) return { in: 3.00, out: 15.00, estimado: true };
  return { in: 0, out: 0, estimado: true };
}
...
/* Em modelos Gemini 3.x o maxOutputTokens é orçamento COMBINADO de raciocínio + resposta,
   e o raciocínio consome quase tudo: sem limitá-lo, a resposta volta vazia.
   Gemini 3+ usa thinkingLevel; 2.5 usa thinkingBudget. Enviar os dois juntos é erro. */
function thinkingConfig() {
  const nivel = CFG.geminiThinking.toLowerCase();
  const g3 = /gemini-3/i.test(CFG.geminiModel);
  if (nivel === 'off') return g3 ? { thinkingLevel: 'minimal' } : { thinkingBudget: 0 };
  if (g3) return { thinkingLevel: ['minimal', 'low', 'medium', 'high'].includes(nivel) ? nivel : 'minimal' };
  return { thinkingBudget: nivel === 'high' ? 8192 : nivel === 'medium' ? 2048 : nivel === 'low' ? 512 : 0 };
}
...
async function callGemini({ system, user, maxTokens = CFG.maxTokens, tools, onDelta, onWarn, label = 'gemini' }) {
  ...
  return withRetry(async (state) => {
    const payload = {
      ...
      generationConfig: {
        temperature: 0.7,
        /* folga: o raciocínio divide este orçamento com o texto */
        maxOutputTokens: Math.max(1024, Math.round(maxTokens * 2.5)),
    ...
    const t0 = Date.now();
    let text = '', tin = 0, tout = 0, pensou = 0, fim = '';
    const refs = [];
    const res = await streamRequest({
      ...
      if (ev.candidates?.[0]?.finishReason) fim = ev.candidates[0].finishReason;
      if (ev.usageMetadata) {
        tin = ev.usageMetadata.promptTokenCount || tin;
        tout = ev.usageMetadata.candidatesTokenCount || tout;
        pensou = ev.usageMetadata.thoughtsTokenCount || pensou;
      }
    });
    if (res.status !== 200) {
      let m = res.body.slice(0, 200);
    ...
    if (!text.trim()) {
      throw new Error(`Gemini devolveu texto vazio (finishReason=${fim || 'desconhecido'}, ${pensou} tokens gastos em raciocínio).`
        + ` Reduza GEMINI_THINKING ou aumente MAX_TOKENS.`);
    }
    return { text: text.trim(), ms: Date.now() - t0, in: tin, out: tout + pensou, pensou, fim, model: CFG.geminiModel, cost: costOf(CFG.geminiModel, tin, tout + pensou), refs };
  }, label, onWarn);
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname.startsWith('/api/')) {
    ...
    /* diagnóstico: chama cada provedor uma vez, com o mínimo de tokens, e devolve o erro cru */
    if (url.pathname === '/api/testar') {
      const provar = async (nome, fn) => {
        const t0 = Date.now();
        try {
          const r = await fn();
          return { ok: true, ms: Date.now() - t0, resposta: r.text.slice(0, 80), tokens: `${r.in}→${r.out}`, raciocinio: r.pensou || 0 };
        } catch (e) {
          return { ok: false, ms: Date.now() - t0, erro: e.message };
        }
      };
      const [claude, gemini] = await Promise.all([
        provar('claude', () => callClaude({ system: 'Responda apenas: OK', user: 'diagnostico', maxTokens: 16 })),
        provar('gemini', () => callGemini({ system: 'Responda apenas: OK', user: 'diagnostico', maxTokens: 400 })),
      ]);
      console.log(`[testar] claude=${claude.ok ? 'ok' : claude.erro} | gemini=${gemini.ok ? 'ok' : gemini.erro}`);
      return json(res, 200, {
        modelos: { claude: CFG.claudeModel, gemini: CFG.geminiModel, raciocinio_gemini: CFG.geminiThinking },
        formato_chave: {
          anthropic: CFG.anthropicKey ? `${CFG.anthropicKey.slice(0, 7)}… (${CFG.anthropicKey.length} caracteres)` : 'ausente',
          gemini: CFG.geminiKey ? `${CFG.geminiKey.slice(0, 5)}… (${CFG.geminiKey.length} caracteres)` : 'ausente',
        },
        claude, gemini,
      });
    }

    /* quais modelos a SUA chave pode usar hoje — o Google aposenta modelos sem aviso */
    if (url.pathname === '/api/modelos') {
      if (!CFG.geminiKey) return json(res, 400, { error: 'GEMINI_API_KEY não configurada' });
      try {
        const r = await httpGet(`https://generativelanguage.googleapis.com/v1beta/models?pageSize=200&key=${encodeURIComponent(CFG.geminiKey)}`);
        let j;
        try { j = JSON.parse(r.body); } catch { return json(res, r.status || 502, { error: r.body.slice(0, 200) }); }
        if (r.status !== 200) return json(res, r.status, { error: j.error?.message || r.body.slice(0, 200) });
        const geram = (j.models || [])
          .filter((m) => (m.supportedGenerationMethods || []).includes('generateContent'))
          .map((m) => m.name.replace(/^models\//, ''))
          .filter((n) => !/embedding|aqa|imagen|veo|tts|image|audio/i.test(n));
        const flash = geram.filter((n) => /flash/i.test(n) && !/lite|preview|exp|thinking/i.test(n));
        return json(res, 200, {
          em_uso: CFG.geminiModel,
          em_uso_disponivel: geram.includes(CFG.geminiModel),
          sugestao: flash.sort().reverse()[0] || geram[0] || null,
          flash_estaveis: flash.sort(),
          todos: geram.sort(),
        });
    ...
    if (url.pathname === '/api/run' && req.method === 'POST') {
      ...
      const emit = (ev, data) => {
        if (ev === 'warn' || ev === 'error') console.log(`[${ev}] ${data.message}`);
        if (!closed && !res.writableEnded) res.write(`event: ${ev}\ndata: ${JSON.stringify(data)}\n\n`);
      };
...
/* 0.0.0.0 por padrão: é o que Render, Fly, Railway e Docker exigem para detectar a porta.
   Atrás de proxy numa VPS, fixe HOST=127.0.0.1 explicitamente. */
const HOST = process.env.HOST || '0.0.0.0';
server.listen(CFG.port, HOST, () => {
  console.log(`UROBOROS v4 ${HOST}:${CFG.port} — ${CFG.clusters} clusters | ${CFG.claudeModel} × ${CFG.geminiModel}`);
  console.log(`  variáveis recebidas: HOST=${process.env.HOST || '(ausente)'} PORT=${process.env.PORT || '(ausente)'}`
    + ` ACCESS_TOKEN=${CFG.accessToken ? 'definida' : '(ausente)'}`
    + ` ANTHROPIC=${CFG.anthropicKey ? 'ok' : '(ausente)'} GEMINI=${CFG.geminiKey ? 'ok' : '(ausente)'}`);
