  ...
  if (url.pathname.startsWith('/api/')) {
    ...
    if (url.pathname === '/api/testar') {
      const provar = async (nome, fn) => {
        ...
        try {
          const r = await fn();
          return { ok: true, ms: Date.now() - t0, resposta: r.text.slice(0, 80), tokens: `${r.in}→${r.out}` };
        } catch (e) {
      ...
      const [claude, gemini] = await Promise.all([
        ...
        provar('gemini', () => callGemini({ system: 'Responda apenas: OK', user: 'diagnostico', maxTokens: 16 })),
      ]);
      console.log(`[testar] claude=${claude.ok ? 'ok' : claude.erro} | gemini=${gemini.ok ? 'ok' : gemini.erro}`);
      return json(res, 200, {
        modelos: { claude: CFG.claudeModel, gemini: CFG.geminiModel },
        formato_chave: {
